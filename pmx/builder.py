...
def make_residue(key, hydrogens=True):
    ...
    if key not in library._aacids:
        raise_(KeyError, "Residue %s not known" % key)
    m = Molecule()
    m.unity = "A"
    m.resname = key
...
def build_chain(sequence, dihedrals=None, hydrogens=True, ss=None, chain_id="X"):
    ...
    try:
        ...
        start = make_residue(
            library._aacids_ext_amber[sequence[0]], hydrogens=hydrogens
        )
    start.set_resid(1)
    ...
    for i, aa in enumerate(sequence[1:]):
        ...
        mol = attach_aminoacid(
            start,
            library._aacids_ext_amber[aa],
            hydrogens=hydrogens,
            phi=phi,
            psi=psi,
            omega=om2,
...
def set_psi(mol, phi):
    ...
    phi = old_div(pi, 180) * phi - pi  # psi is defined with N'
    delta = phi - dih
    r = Rotation(CA.x, C.x)
...
def attach_aminoacid(mol, resname, hydrogens=True, omega=180.0, phi=-139.0, psi=135.0):
    ...
    if H:
        ...
        x = 1.0 / linalg.norm(v1)
        H.x = N.x + v1 * x
        H.x = r.apply(H.x, 2 * pi / 3.0)
