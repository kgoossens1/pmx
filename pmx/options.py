...
class Option(OptionBase):
    ...
    def __error(self, arg):
        print(
            'Error: Option "%s" (%s) is not compatible with argument ->'
            % (self.flag, self.type),
            arg,
            file=sys.stderr,
        )
...
class Commandline(object):
    ...
    def __consistency_check(self):
        ...
        for opt in self.options + self.fileoptions:
            if opt.flag in olist:
                print(
                    'Error: Option flag "%s" defined multiple times' % opt.flag,
                    file=sys.stderr,
                )
                sys.exit(1)
    ...
    def __check_flags(self):
        for flag in self.flag_list:
            ...
            if n != 1:
                print(
                    'Error: Flag "%s" appears %d times in commandline' % (flag, n),
                    file=sys.stderr,
                )
                sys.exit(1)
    ...
    def __check_for_unparsed_args(self):
        ...
        for i, arg in enumerate(self.cmdline):
            if i not in self.parsed_opts:
                print(
                    'Error: Unknown argument "%s" in commandline' % (arg),
                    file=sys.stderr,
                )
                error_occured = True
        if error_occured:
...
if __name__ == "__main__":
    ...
    options = [
        ...
        Option(
            ...
            "rvec",
            [1, 2, 3],
            "some vector that does wonderful things and returns always segfaults",
        ),
    ]
    ...
    files = [
        FileOption(
            ...
            ["pdb"],
            "xx.pdb",
            "pdb file with many other things we do not care about at the moment. but anyway... nice",
        ),
        FileOption("-pdb2", "r/m", ["pdb"], "xx.pdb", "pdb file"),
        FileOption("-pdb3", "w", ["pdb"], "xx.pdb", "pdb file"),
    ]

    help_text = ["does a lot of useful stuff", "most of the time"]
