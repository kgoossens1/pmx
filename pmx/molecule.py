...
class Molecule(Atomselection):
    ...
    def get_psi(self, degree=False):
        ...
        if not degree:
            ...
            return old_div(dih * 180, pi)

    def set_psi(self, degree, propagate=True):
        psi = self.get_psi()
        diff = degree * pi / 180.0 - psi
    ...
    def append(self, atom):
        ...
        if not isinstance(atom, Atom):
            raise_(TypeError, "%s is not an Atom instance" % str(atom))
        else:
            n = len(self.atoms)
            if n == 0:
                self.atoms.append(atom)
...
class Mol2Molecule(object):
    ...
    def write(self, fp=sys.stdout):
        ...
        for atom in self.atoms:
            print(
                "%7d %-8s %9.4f %9.4f %9.4f %-6s %3d %-8s %9.4f"
                % (
                    atom.id,
                    atom.symbol,
                    atom.x[0],
