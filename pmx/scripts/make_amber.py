...
help_text = (
    "This script reads a structure file",
...
for res in cysl:
    ...
    if not hg:  # no hydrogen
        ...
        if ss_bond:
            ...
            rr = "CYS2"
            print(
                "Residue %d-%s (chain %s) will become %s"
                % (res.id, res.resname, res.chain_id, rr)
            )
            res.set_resname(rr)
        else:
            print(
                "Residue %d-%s (chain %s) will become %s"
                % (res.id, res.resname, res.chain_id, "CYM")
            )
            res.set_resname("CYM")
        ...
        print(
            "Residue %d-%s (chain %s) will become %s"
            % (res.id, res.resname, res.chain_id, res.resname)
        )
...
for res in lysl:
    ...
    print(
        "Residue %d-%s (chain %s) will become %s"
        % (res.id, "LYS", res.chain_id, res.resname)
    )
...
for res in hisl:
    ...
    print(
        "Residue %d-%s (chain %s) will become %s"
        % (res.id, "HIS", res.chain_id, res.resname)
    )
...
for res in aspl:
    ...
    print(
        "Residue %d-%s (chain %s) will become %s"
        % (res.id, "ASP", res.chain_id, res.resname)
    )
...
for res in glul:
    ...
    print(
        "Residue %d-%s (chain %s) will become %s"
        % (res.id, "GLU", res.chain_id, res.resname)
    )
