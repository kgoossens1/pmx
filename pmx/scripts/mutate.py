...
def check_residue_name(res):
    if res.resname == "LYS":
        ...
        if res.has_atom("HD1") and res.has_atom("HE2"):
            res.set_resname("HIP")
...
def select_residue(m):
    ...
    while not selected_residue_id:
        ...
        if selected_residue_id is not None and selected_residue_id not in valid_ids:
            print(
                "Residue id %d not in range %d-%d -> Try again"
                % (selected_residue_id, 1, len(residues))
            )
            selected_residue_id = None
...
def select_nuc_mutation(residue):
    ...
    while aa is None:
        ...
        if aa:
            print(
                "Will apply mutation %s->%s on residue %s-%d"
...
def select_aa_mutation(residue, ffpath):
    ...
    while aa is None:
        ...
        if len(aa) != 1 and len(aa) != 3 and len(aa) != 4:
            ...
            (len(aa) == 1 and aa not in ol + ["B", "J", "O", "X", "Z"])
            or (len(aa) == 3 and aa not in tl)
            or (len(aa) == 4 and aa not in tl)
        ):
            sys.stdout.write(
                'Unknown aa "%s"!\nThree- or one-letter code (or four-letter for ff specific residues): '
                % aa
...
def convert_aa_name(aa):
    if len(aa) == 1:
        return aa.upper()
    elif len(aa) == 2 and aa == "CM":
        return aa  # special case for two letter code
    elif len(aa) == 3:
        return ext_one_letter[aa.upper()]
    elif len(aa) == 4:
        return ext_one_letter[aa.upper()]
    else:
        raise UnkownResidueError(aa)
...
def rename_to_match_library(res):
    ...
    for atom in atoms:
        ...
        if ("S2" in atom.resname) and (atom.name == "HG1"):
            atom.name = "HG"
        if ("SP1" in atom.resname) and (
            atom.name == "HG1"
        ):  # phosphoserine in charmm36
            atom.name = "HG"
        if ("SP2" in atom.resname) and (
            atom.name == "HG1"
        ):  # phosphoserine in charmm36
            atom.name = "HG"
        # for cysteine
...
def apply_nuc_mutation(m, residue, new_nuc_name, mtp_file, bRNA=False):
    ...
    #    hybrid_residue_name = residue.resname+new_nuc_name
    hybrid_residue_name, resname1, resname2 = get_nuc_hybrid_resname(
        residue, new_nuc_name, bRNA
    )
    print(
        "log_> Residue to mutate: %d | %s | %s "
        % (residue.id, residue.resname, residue.chain_id)
    )
    print("log_> Mutation to apply: %s->%s" % (residue.resname[1], new_nuc_name))
    print("log_> Hybrid residue name: %s" % hybrid_residue_name)
    hybrid_res, bonds, imps, diheds, rotdic = get_hybrid_residue(
        hybrid_residue_name, mtp_file
    )
    #    hybrid_res.nm2a()
    ...
    print(
        "log_> Inserted hybrid residue %s at position %d (chain %s)"
...
def get_hybrid_residue(residue_name, mtp_file="ffamber99sb.mtp"):
    ...
    resi, bonds, imps, diheds, rotdic = read_mtp_entry(
        residue_name, filename=mtp_file, version="new"
    )
    if len(resi.atoms) == 0:
...
def rename_ile(residue):
    dic = {"CD": "CD1", "HD1": "HD11", "HD2": "HD12", "HD3": "HD13"}
...
def main(argv):
    ...
    files = [
        ...
        FileOption(
            ...
            ["pdb", "gro"],
            "proteinB.pdb",
            "input structure file of the Bstate (optional)",
        ),
        ...
        FileOption(
            ...
            ["txt"],
            "mutations.txt",
            "text file with mutations to insert",
        ),
    ]

    help_text = (
        "This script applies mutations of residues in a structure file ",
