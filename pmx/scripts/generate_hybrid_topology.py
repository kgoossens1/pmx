...
def dump_atoms_and_exit(msg, atoms):
    ...
    print(
        "err_>  name      resname      atomtype       atomtypeB       bondtype       bondtypeB",
        file=sys.stderr,
    )
    for atom in atoms:
        print(
...
def atoms_morphe(atoms):
    for atom in atoms:
        if atom.atomtypeB is not None and (
            atom.q != atom.qB or atom.m != atom.mB or atom.atomtype != atom.atomtypeB
        ):
            return True
    return False
...
def find_bonded_entries(topol):
    ...
    for b in topol.bonds:
        ...
        if a1.atomtypeB is not None or a2.atomtypeB is not None:
            ...
            if astate == None:
                print(
                    "Error A: No bond entry found for astate %s-%s (%s-%s -> %s-%s)"
                    % (a1.name, a2.name, a1.atomtype, a2.atomtype, a1.type, a2.type)
                )
            ...
            if bstate == None:
                print(
                    "Error B: No bond entry found for bstate %s-%s (%s-%s -> %s-%s)"
                    % (a1.name, a2.name, a1.atomtypeB, a2.atomtypeB, a1.typeB, a2.typeB)
                )
                error_occured = True
            if error_occured:
                sys.exit(1)
...
def find_angle_entries(topol):
    ...
    for a in topol.angles:
        ...
        bstate = None
        if (
            a1.atomtypeB is not None
            or a2.atomtypeB is not None
            or a3.atomtypeB is not None
        ):
            ...
            if "D" in A and "D" in B:  # fake angle
                ...
                bstate = topol.BondedParams.get_angle_param(
                    a1.typeB, a2.typeB, a3.typeB
                )
                astate = bstate
                a.extend([astate, bstate])
            elif A == "AAA" and B == "AAA":
                if (
                    a1.atomtypeB != a1.atomtype
                    or a2.atomtypeB != a2.atomtype
                    or a3.atomtypeB != a3.atomtype
                ):
                    astate = topol.BondedParams.get_angle_param(
...
def find_dihedral_entries(topol, rlist, rdic, dih_predef_default):
        ...
        if len(d) >= 6:
            ...
            if atoms_morphe([a1, a2, a3, a4]):
                ...
                if A != "AAAA" and B != "AAAA":
                    ...
                    bstate = []
                    if A == "AAAA" and B != "AAAA":
                        foo = topol.BondedParams.get_dihedral_param(
                            a1.type, a2.type, a3.type, a4.type, func
                        )
                        # need to check if the dihedral has torsion pre-defined
                        counter = check_dih_ILDN_OPLS(
                            topol, rlist, rdic, a1, a2, a3, a4
                        )
                        if counter == 42:
                            continue
                        ...
                        foo = topol.BondedParams.get_dihedral_param(
                            a1.typeB, a2.typeB, a3.typeB, a4.typeB, func
                        )
                        # need to check if the dihedral has torsion pre-defined
                        counter = check_dih_ILDN_OPLS(
                            topol, rlist, rdic, a1, a2, a3, a4
                        ...
                        if val == "":
                            if undef != 4:
                                astate = topol.BondedParams.get_dihedral_param(
                                    a1.type, a2.type, a3.type, a4.type, func
                                )
                        else:
                            if undef != 4:
                        ...
                        if undef != 3:
                            bstate = topol.BondedParams.get_dihedral_param(
                                a1.typeB, a2.typeB, a3.typeB, a4.typeB, func
                            )
                        #                        else:
                        # 			    if (undef != 3):
                    ...
                    if astate == None:
                        print(
                            "Error: No dihedral angle found (state A: predefined state B) for:"
                        )
                    ...
                    if bstate == None:
                        print(
                            "Error: No dihedral angle found (state B: predefined state A) for:"
                        )
                        print(a1.resname, a2.resname, a3.resname, a4.resname)
                        print(a1.name, a2.name, a3.name, a4.name, func)
                        print(a1.atomtype, a2.atomtype, a3.atomtype, a4.atomtype)
    ...
    print(
        "log_> Making dihedrals for state B -> %d dihedrals with perturbed atoms"
        % count
    )
        ...
        for d in imp + dih:
            ...
            for dx in topol.dihedrals:
                ...
                if (
                    ...
                    encountered = is_ildn_dih_encountered(
                        ildn_used, foobar, encountered
                    )
                    ...
                    A, B = check_case(al[:4])
                    paramA = topol.BondedParams.get_dihedral_param(
                        al[0].type, al[1].type, al[2].type, al[3].type, func
                    )
                    paramB = topol.BondedParams.get_dihedral_param(
                        al[0].typeB, al[1].typeB, al[2].typeB, al[3].typeB, func
                    )
...
def get_hybrid_residue(residue_name, mtp_file="ffamber99sb.mtp", version="old"):
    ...
    resi, bonds, imps, diheds, rotdic = read_mtp_entry(
        residue_name, filename=mtp_file, version=version
    )
    if len(resi.atoms) == 0:
...
def is_hybrid_residue(resname):
    if resname in _perturbed_nucleotides or resname[1] == "2":
        return True
...
def __add_extra_DNA_RNA_impropers(topol, rlist, func_type, stateA, stateB):
    ...
    for r in rlist:
        if r.resname in ["DAT", "DAC", "DGC", "DGT", "RAU", "RAC", "RGC", "RGU"]:
            print(
                "Adding extra improper dihedrals for residue %d-%s" % (r.id, r.resname)
            )
...
def main(argv):
    ...
    options = [
        Option(
            "-split", "bool", False, "Write splitted topologies for vdw and q morphes"
        ),
    ...
    files = [
        ...
        FileOption(
            "-o", "w", ["top", "itp"], "newtop.top", "Topology or ITP output file "
        ),
    ...
    dih_predef_default = []
    find_predefined_dihedrals(
        topol, rlist, rdic, ffbonded_file, dih_predef_default, cmdl["-ff"]
    )
