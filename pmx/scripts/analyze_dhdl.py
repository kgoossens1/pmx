...
def parse_dgdl_files(lst, lambda0=0, invert_values=False):
    ...
    first_w, ndata = integrate_dgdl(
        lst[imax], lambda0=lambda0, invert_values=invert_values
    )
    w_list = [first_w]
    for idx, f in enumerate(lst[0:]):
        ...
        w, _ = integrate_dgdl(
            f, ndata=ndata, lambda0=lambda0, invert_values=invert_values
        )
        if w is not None:
            w_list.append(w)
    ...
    if ndata != -1 and len(r) != ndata:
        try:
            if "incomplete_file" in r:
                print(
                    " !! Skipping %s (incomplete file, probably simulation crashed)\n"
                    % fn
                )
            else:
                print(
                    " !! Skipping %s ( read %d data points, should be %d )"
                    % (fn, len(r), ndata)
                )
        except:
            print(" !! Skipping %s " % (fn))
...
def parse_options():
    ...
    parser = argparse.ArgumentParser(
        description="Calculates free energies "
    ...
    parser.add_argument(
        ...
        dest="outfn",
        type=str,
        help="Filename of output result file. Default is " '"results.txt."',
        default="results.txt",
    )
    parser.add_argument(
        "-b",
    ...
    parser.add_argument(
        ...
        dest="precision",
        type=int,
        help="The decimal precision of the screen/file output." " Default is 2.",
        default=2,
    )
    parser.add_argument(
        "--units",
    ...
    parser.add_argument(
        ...
        dest="nbins",
        type=int,
        help="Number of histograms bins for the plot. " "Default is 10.",
        default=10,
    )
    parser.add_argument(
        "--dpi",
...
def main(args):
    ...
    if "cgi" in methods and "AB" in statesProvided:
        ...
        _tee(
            out,
            "  CGI: Forward Gauss mean = {m:8.{p}f} {u} "
            "std = {s:8.{p}f} {u}".format(
                m=cgi.mf * unit_fact, s=cgi.devf * unit_fact, p=prec, u=units
            ),
        )
        _tee(
            out,
            "  CGI: Reverse Gauss mean = {m:8.{p}f} {u} "
            "std = {s:8.{p}f} {u}".format(
                m=cgi.mr * unit_fact, s=cgi.devr * unit_fact, p=prec, u=units
            ),
        )
        ...
        _tee(
            out,
            "  CGI: dG = {dg:8.{p}f} {u}".format(
                dg=cgi.dg * unit_fact, p=prec, u=units
            ),
        )
        _tee(
            out,
            "  CGI: Std Err (bootstrap:parametric) = {e:8.{p}f} {u}".format(
                e=cgi.err_boot1 * unit_fact, p=prec, u=units
            ),
        )
        ...
        if nboots > 0:
            _tee(
                out,
                "  CGI: Std Err (bootstrap) = {e:8.{p}f} {u}".format(
                    e=cgi.err_boot2 * unit_fact, p=prec, u=units
                ),
            )
        ...
        if nblocks > 1:
            _tee(
                out,
                "  CGI: Std Err (blocks) = {e:8.{p}f} {u}".format(
                    e=cgi.err_blocks * unit_fact, p=prec, u=units
                ),
            )
    ...
    if do_ks_test and "AB" in statesProvided:
        ...
        if bOk0:
            ...
            _tee(
                out,
                "             ---> KS-Test Failed. sqrt(N)*Dmax = %4.2f,"
    ...
    if "bar" in methods and "AB" in statesProvided:
        ...
        _tee(
            out,
            "  BAR: dG = {dg:8.{p}f} {u}".format(
                dg=bar.dg * unit_fact, p=prec, u=units
            ),
        )
        _tee(
            out,
            "  BAR: Std Err (analytical) = {e:8.{p}f} {u}".format(
                e=bar.err * unit_fact, p=prec, u=units
            ),
        )
        ...
        if nboots > 0:
            _tee(
                out,
                "  BAR: Std Err (bootstrap)  = {e:8.{p}f} {u}".format(
                    e=bar.err_boot * unit_fact, p=prec, u=units
                ),
            )
        if nblocks > 1:
            _tee(
                out,
                "  BAR: Std Err (blocks)  = {e:8.{p}f} {u}".format(
                    e=bar.err_blocks * unit_fact, p=prec, u=units
                ),
            )
    ...
    if "jarz" in methods or "AB" not in statesProvided:
        ...
        if "A" in statesProvided:
            _tee(
                out,
                "  JARZ: dG Forward = {dg:8.{p}f} {u}".format(
                    dg=jarz.dg_for * unit_fact, p=prec, u=units
                ),
            )
        if "B" in statesProvided:
            _tee(
                out,
                "  JARZ: dG Reverse = {dg:8.{p}f} {u}".format(
                    dg=jarz.dg_rev * unit_fact, p=prec, u=units
                ),
            )
        if "AB" in statesProvided:
            _tee(
                out,
                "  JARZ: dG Mean    = {dg:8.{p}f} {u}".format(
                    dg=jarz.dg_mean * unit_fact, p=prec, u=units
                ),
            )
        if nboots > 0:
            if "A" in statesProvided:
                _tee(
                    out,
                    "  JARZ: Std Err Forward (bootstrap) = {e:8.{p}f} {u}".format(
                        e=jarz.err_boot_for * unit_fact, p=prec, u=units
                    ),
                )
            if "B" in statesProvided:
                _tee(
                    out,
                    "  JARZ: Std Err Reverse (bootstrap) = {e:8.{p}f} {u}".format(
                        e=jarz.err_boot_rev * unit_fact, p=prec, u=units
                    ),
                )
        ...
        if nblocks > 1:
            if "A" in statesProvided:
                _tee(
                    out,
                    "  JARZ: Std Err Forward (blocks) = {e:8.{p}f} {u}".format(
                        e=jarz.err_blocks_for * unit_fact, p=prec, u=units
                    ),
                )
            if "B" in statesProvided:
                _tee(
                    out,
                    "  JARZ: Std Err Reverse (blocks) = {e:8.{p}f} {u}".format(
                        e=jarz.err_blocks_rev * unit_fact, p=prec, u=units
                    ),
                )
        ...
        if "A" in statesProvided:
            _tee(
                out,
                "  JARZ_Gauss: dG Forward = {dg:8.{p}f} {u}".format(
                    dg=jarzGauss.dg_for * unit_fact, p=prec, u=units
                ),
            )
        if "B" in statesProvided:
            _tee(
                out,
                "  JARZ_Gauss: dG Reverse = {dg:8.{p}f} {u}".format(
                    dg=jarzGauss.dg_rev * unit_fact, p=prec, u=units
                ),
            )
        if "AB" in statesProvided:
            _tee(
                out,
                "  JARZ_Gauss: dG Mean    = {dg:8.{p}f} {u}".format(
                    dg=(jarzGauss.dg_for + jarzGauss.dg_rev) / 2.0 * unit_fact,
                    p=prec,
                    u=units,
                ),
            )
        if "A" in statesProvided:
            _tee(
                out,
                "  JARZ_Gauss: Std Err (analytical) Forward = {dg:8.{p}f} {u}".format(
                    dg=jarzGauss.err_for * unit_fact, p=prec, u=units
                ),
            )
        if "B" in statesProvided:
            _tee(
                out,
                "  JARZ_Gauss: Std Err (analytical) Reverse = {dg:8.{p}f} {u}".format(
                    dg=jarzGauss.err_rev * unit_fact, p=prec, u=units
                ),
            )
        if nboots > 0:
            if "A" in statesProvided:
                _tee(
                    out,
                    "  JARZ_Gauss: Std Err Forward (bootstrap) = {e:8.{p}f} {u}".format(
                        e=jarzGauss.err_boot_for * unit_fact, p=prec, u=units
                    ),
                )
            if "B" in statesProvided:
                _tee(
                    out,
                    "  JARZ_Gauss: Std Err Reverse (bootstrap) = {e:8.{p}f} {u}".format(
                        e=jarzGauss.err_boot_rev * unit_fact, p=prec, u=units
                    ),
                )
        ...
        if nblocks > 1:
            if "A" in statesProvided:
                _tee(
                    out,
                    "  JARZ_Gauss: Std Err Forward (blocks) = {e:8.{p}f} {u}".format(
                        e=jarzGauss.err_blocks_for * unit_fact, p=prec, u=units
                    ),
                )
            if "B" in statesProvided:
                _tee(
                    out,
                    "  JARZ_Gauss: Std Err Reverse (blocks) = {e:8.{p}f} {u}".format(
                        e=jarzGauss.err_blocks_rev * unit_fact, p=prec, u=units
                    ),
                )
    ...
    if args.pickle:
        print(
            "   NOTE: units of results in pickled files are as in the\n"
