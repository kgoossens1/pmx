...
standard_pair_listD = [("N", "N"), ("H", "H"), ("CA", "CA"), ("C", "C"), ("O", "O")]
    ...
    for atom in mol.atoms:
        if atom.order > a3.order:
            if a3.long_name[3] == " ":
                ...
                if atom.long_name[3] == a3.long_name[3] or atom.long_name[3] == " ":
                    #                    print 'rotating', atom.name
                    atom.x = r.apply(atom.x, -rot)
...
def get_atoms_by_order_and_branch(mol, order, branch, merged_atoms):
    ...
    for atom in mol.atoms:
        if atom.order == order and atom not in merged_atoms:
            if (
                atom.branch in [0, branch]
                or atom.branch < mol_branch[mol.real_resname] + 1
            ):
                res.append(atom)
    return res
    ...
    for i, atom in enumerate(atom_list):
        if atom not in merged_atoms:
            if bH2heavy == False:
                ...
                if atom1.atomtype.startswith("H") and (
                    not atom.atomtype.startswith("H")
                ):
                    continue
                if atom.atomtype.startswith("H") and (
                    not atom1.atomtype.startswith("H")
                ):
                    continue
            ...
            for i in range(2, 8):
                ...
                for at1 in atoms1:
                    if last_atom_is_morphed(at1, merged_atoms1):
                        ...
                        aa, d = find_closest_atom(
                            at1, candidates, merged_atoms2, bH2heavy
                        )
                        if aa:
                            merged_atoms2.append(aa)
                            merged_atoms1.append(at1)
...
def generate_dihedral_entries(im1, im2, r, pairs):
    ...
    for i1 in im1:
        if i1 not in done_i1:
            ...
            if i1[4] == "":
                ...
                if (
                    ("gone" in i1[0].nameB)
                    or ("gone" in i1[1].nameB)
                    or ("gone" in i1[2].nameB)
                    or ("gone" in i1[3].nameB)
                ):
                    im_new.append("default-A")
                else:
                    im_new.append("un")
    ...
    for i2 in im2:
        if i2 not in done_i2:
            ...
            if i2[4] == "":
                if (
                    (i2[0].name.startswith("D"))
                    or (i2[1].name.startswith("D"))
                    or (i2[2].name.startswith("D"))
                    or (i2[3].name.startswith("D"))
                ):
                    im_new.append("default-B")
                else:
                    im_new.append("un")
...
def write_rtp(fp, r, ii_list, dihi_list, neigh_bonds, cmap):
    ...
    for atom in r.atoms:
        print(
            "%6s   %-15s  %8.5f  %d" % (atom.name, atom.atomtype, atom.q, cgnr), file=fp
        )
        cgnr += 1
    print("\n [ bonds ]", file=fp)
    for atom in r.atoms:
    ...
    for ii in ii_list:
        if not ii[4].startswith("default"):
            print(
                "%6s  %6s  %6s  %6s  %-25s"
                % (ii[0].name, ii[1].name, ii[2].name, ii[3].name, ii[4]),
                file=fp,
            )
        else:
            print(
                "%6s  %6s  %6s  %6s "
                % (ii[0].name, ii[1].name, ii[2].name, ii[3].name),
                file=fp,
            )
...
def write_mtp(fp, r, ii_list, rotations, dihi_list):
    ...
    for atom in r.atoms:
        print(
            "%6s %10s -> %6s %10s"
            % (atom.name, atom.atomtype, atom.nameB, atom.atomtypeB),
            file=fp,
        )
    print("\n [ atoms ]", file=fp)
    cgnr = 1
    for atom in r.atoms:
        ...
        print(
            "%8s %10s %10.6f %6d %10.6f %10s %10.6f %10.6f  %-10s"
            % (
                atom.name,
                atom.atomtype,
                atom.q,
    ...
    if rotations:
        ...
        for rot in rotations:
            print(
                "  %s-%s %s"
                % (rot[0].name, rot[1].name, " ".join([a.name for a in rot[2:]])),
                file=fp,
            )
        print(file=fp)
        ...
        if atom.atomtype[0:3] == "DUM":
            if atom.atomtype not in types:
                if "opls" in ffnamelower:
                    ...
                    ofile.write(
                        "%-13s\t\t%3s\t0\t%4.2f\t   0.0000  A   0.00000e+00 0.00000e+00\n"
                        % (atom.atomtype, foo[0], atom.m)
                    )
                else:
                    ofile.write(
                        "%-10s\t0\t%4.2f\t   0.0000  A   0.00000e+00 0.00000e+00\n"
                        % (atom.atomtype, atom.m)
                    )
                types.append(atom.atomtype)
        if atom.atomtypeB[0:3] == "DUM":
...
options = [
    Option(
        ...
        "string",
        "charmm",
        "force field type (charmm, amber99sb, amber99sb*-ildn, oplsaa",
    ),
    Option("-align", "bool", True, "align side chains"),
    Option("-cbeta", "bool", False, "morphing up to Cbeta atom"),
    Option(
        "-H2heavy", "bool", True, "allow morphing between hydrogens and heavy atoms"
    ),
...
help_text = (
    "The script creates hybrid structure (.pdb) and topology database entries (.rtp, .mtp).",
...
if bDNA:
    if (
        (("5" in r1.resname) and ("5" not in r2.resname))
        or (("3" in r1.resname) and ("3" not in r2.resname))
        or (("5" in r2.resname) and ("5" not in r1.resname))
        or (("3" in r2.resname) and ("3" not in r1.resname))
    ):
        print(
            "Cannot mutate terminal nucleic acid to non-terminal or a terminal of the other end (e.g. 5' to 3')"
        )
        sys.exit(0)
    if (
        r1.resname in use_standard_dna_pair_list
        and r2.resname in use_standard_dna_pair_list[r1.resname]
    ):
        print("PURINE <-> PYRIMIDINE")
        if bCharmm:
            atom_pairs, dummies = make_predefined_pairs(
                r1, r2, standard_dna_pair_list_charmm
            )
        else:
            atom_pairs, dummies = make_predefined_pairs(r1, r2, standard_dna_pair_list)
    elif (
        r1.resname in use_standard_dna_5term_pair_list
        and r2.resname in use_standard_dna_5term_pair_list[r1.resname]
    ):
        print("PURINE <-> PYRIMIDINE: 5term")
        if bCharmm:
            atom_pairs, dummies = make_predefined_pairs(
                r1, r2, standard_dna_5term_pair_list_charmm
            )
        else:
            atom_pairs, dummies = make_predefined_pairs(
                r1, r2, standard_dna_5term_pair_list
            )
    elif (
        r1.resname in use_standard_dna_3term_pair_list
        and r2.resname in use_standard_dna_3term_pair_list[r1.resname]
    ):
        print("PURINE <-> PYRIMIDINE: 3term")
        if bCharmm:
            atom_pairs, dummies = make_predefined_pairs(
                r1, r2, standard_dna_3term_pair_list_charmm
            )
        else:
            atom_pairs, dummies = make_predefined_pairs(
                r1, r2, standard_dna_3term_pair_list
            )
    else:
    ...
    if (
        ...
        sys.exit(0)
    if (
        r1.resname in use_standard_rna_pair_list
        and r2.resname in use_standard_rna_pair_list[r1.resname]
    ):
        print("PURINE <-> PYRIMIDINE")
        #        if bCharmm :
        ...
        r1.resname in use_standard_rna_5term_pair_list
        and r2.resname in use_standard_rna_5term_pair_list[r1.resname]
    ):
        print("PURINE <-> PYRIMIDINE: 5term")
        #        if bCharmm :
        ...
        # 	else:
        atom_pairs, dummies = make_predefined_pairs(
            r1, r2, standard_rna_5term_pair_list
        )
    elif (
        r1.resname in use_standard_rna_3term_pair_list
        and r2.resname in use_standard_rna_3term_pair_list[r1.resname]
    ):
        print("PURINE <-> PYRIMIDINE: 3term")
        #        if bCharmm :
        ...
        # 	else:
        atom_pairs, dummies = make_predefined_pairs(
            r1, r2, standard_rna_3term_pair_list
        )
    else:
        print("PURINE <-> PURINE	PYRIMIDINE <-> PYRIMIDINE")
        atom_pairs, dummies = make_pairs(
            r1, r2, bCharmm, bH2heavy, bDNA=False, bRNA=True
        )
####### amino acids #########
# ring-res 2 ring-res
elif (
    r1.resname in use_standard_pair_list
    and r2.resname in use_standard_pair_list[r1.resname]
):
    print("ENTERED STANDARD")
    if bCharmm:
        if cbeta:
            atom_pairs, dummies = make_predefined_pairs(
                r1, r2, standard_pair_list_charmmC
            )
        else:
            atom_pairs, dummies = make_predefined_pairs(
                r1, r2, standard_pair_list_charmm
            )
    else:
        if cbeta:
...
# ringed residues by atom names
    ...
    if cbeta:
        ...
        # make_predefined_pairs( r1, r2, standard_pair_list)
...
# ring-res 2 non-ring-res
    ...
    if bCharmm:
        if cbeta:
            ...
            atom_pairs, dummies = make_predefined_pairs(
                r1, r2, standard_pair_list_charmmB
            )
    else:
        if cbeta:
            atom_pairs, dummies = make_predefined_pairs(r1, r2, standard_pair_listC)
    ...
    if cbeta:
        if (r1.resname == "GLY") or (r2.resname == "GLY"):
            if bCharmm:
                atom_pairs, dummies = make_predefined_pairs(
                    r1, r2, standard_pair_list_charmmD
                )
            else:
                atom_pairs, dummies = make_predefined_pairs(r1, r2, standard_pair_listD)
        else:
