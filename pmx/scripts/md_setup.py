...
def run_command(func, string):
    ...
    if status != 0:
        ...
        print(
            "Output written to %s" % os.path.abspath(os.path.join(".", err_file)),
            file=sys.stderr,
        )
        fp = open(err_file, "w")
        print(s, file=fp)
...
class MD(object):
    ...
    def write_residue_map(self, model):
        ...
        for r in model.residues:
            ...
            print(
                "%d|%s|%s -> %d|%s|%s"
                % (r.orig_id, r.resname, r.old_chain_id, r.id, r.resname, chain),
                file=fp,
            )
        fp.close()
    ...
    def generate_topology(self, pdb_in):
        ...
        run = "pdb2gmx -f %s -water %s -ff %s -o gmx.pdb " % (
            pdb_in,
            self.water,
            self.ff,
        )
    ...
    def generate_sim_box(self):
        if self.princ:
            ...
            run = "editconf -f tmp.pdb -o ed.pdb -d %g -bt %s " % (
                self.box_size,
                self.box_type,
            )
            run_command(generate_sim_box, run)
        else:
            run = "editconf -f gmx.pdb -o ed.pdb -d %g -bt %s " % (
                self.box_size,
                self.box_type,
            )
    ...
    def compact_repr(self, pdb_in, pdb_out):
        run = "echo 0 | trjconv -f %s -s topol.tpr -ur compact -pbc mol -o %s" % (
            pdb_in,
            pdb_out,
        )
...
class FreeEnergyMD(MD):
    ...
    def make_mutation(self, path, affected_chains):
        os.chdir(path)
        run = (
            "~/software/pmx/scripts/mutate_beta45.py -f %s -script mutations.txt -o mut.pdb"
            % self.md_in_conf
        )
...
class DiscreteTI(FreeEnergyMD):
    ...
    def setup_runs(self, path):
        ...
        for lda in self.lambda_steps:
            ...
            run = "grompp -f ../%s -c %s/em.pdb -o %s/topol.tpr" % (
                run_mdp,
                run_dir,
                run_dir,
            )
...
def main(argv):
    ...
    options = [
        ...
        Option(
            ...
            "string",
            "triclinic",
            "box geometry (triclinic, octahedron, dodecahedron)",
        ),
        ...
        Option(
            "-dti_run_time", "float", 10.0, "Simulation time [ns] at each lambda point"
        ),
        ...
        Option(
            ...
            "float",
            50.0,
            "Simulation time [ns] for crooks equilibrium runs",
        ),
        Option(
            "-skip_md_setup",
            "bool",
            False,
            "skip md setup and use -f as starting configuration for free energy runs",
        ),
    ]
    ...
    files = [
        ...
        FileOption(
            ...
            ["mdp"],
            "template",
            "template run mdp file for crooks equilibrium runs",
        ),
        FileOption(
            "-dti_mdp",
            "r/o",
            ["mdp"],
            "template",
            "template run mdp file for discrete TI calculations",
        ),
        FileOption(
            "-min_mdp",
            "r/o",
            ["mdp"],
            "em",
            "template minimization mdp file ( for TI or Crooks )",
        ),
        FileOption(
            "-lambda_steps",
            "r/o",
            ["txt"],
            "lambda_steps",
            "text file with lambda steps for DTI runs",
        ),
    ]
    ...
    if bVsite and bFreeEnergy:
        print(
            "Error: Cannot use virtual sites in free energy calculations !",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not cmdl["-skip_md_setup"]:
        md = MD(
            pdb_in=pdb_in, water=water, conc=conc, box_type=box_type, box_size=box_size
        )
        md.setup()
