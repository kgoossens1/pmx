...
class graphOptions(object):
    def __init__(self, nodeDeg, bMST=True, filename=None):
        ...
        self.opt["dpi"] = 300
        # spring, random, shell, spectral, circular, fruchterman_reingold
        self.opt["layout"] = "spring"
        # node options
        self.opt["nodeSizeScale"] = 100
        self.opt["nodeSizeSlope"] = 0.2  # scaling for the node size
        self.opt["nodeShape"] = "o"  # s o ^ > v < d p h 8
        # nodeDeg, b: blue, g: green, r: red, c: cyan, m: magenta, y: yellow, k: black, w: white
        self.opt["nodeColor"] = nodeDeg
        # http://wiki.scipy.org/Cookbook/Matplotlib/Show_colormaps
        self.opt["nodeCMAP"] = "hot"
...
def identifyRefLig(mat, cmdlopt, nodeNames):
    ...
    if cmdlopt.is_set:
        ...
        if refName not in nodeNames:
            print(
                "The name provided for the reference ligand is not found among the molecules: ",
                refName,
            )
            sys.exit(1)
    else:
...
def plotMolTransparent(pdb, filename):
    ...
    mol = Chem.MolFromPDBFile(pdbName, removeHs=False, sanitize=False)
    os.remove(pdbName)
    newmol = Chem.Draw.rdMolDraw2D.PrepareMolForDrawing(
        mol, forceCoords=True, kekulize=True
    )
    ...
    for pdb1 in pdbs:
        ...
        for pdb2 in pdbs:
            if pdb2 in visited:
                continue
            callS = (
                callString + " -i1 " + pdb1 + " -i2 " + pdb2 + " -score " + outMatFile
            )
            print(callS)
            os.system(callS)
...
def main(argv):
    ...
    desc = (
        "Build a Minimum Spanning Tree or a Tree based on a reference ligand suggesting the alchemical pairs of ligands to morphe.",
        "",
    ...
    files = [
        ...
        FileOption(
            "-ormsd", "w/o", ["dat"], "outRMSD.dat", "output dissimilarity matrix"
        ),
        FileOption(
            "-oNodes",
            "w/o",
            ["dat"],
            "nodeNames.dat",
            "output names of the graph nodes",
        ),
        FileOption(
            "-oEdges", "w", ["dat"], "edges.dat", "output names of the graph edges"
        ),
        FileOption(
            "-iNodes", "r/o", ["dat"], "nodeNames.dat", "read names of the graph nodes"
        ),
        ...
        FileOption(
            ...
            ["dat"],
            "graphOpt.dat",
            "graph options file (see example)",
        ),
        FileOption(
            "-ligImg",
            "r/w/o",
    ...
    options = [
        Option(
            ...
            "bool",
            "True",
            "build a minimum spanning tree, otherwise use one reference ligand",
        ),
        Option(
            "-refName",
            "string",
            "none",
            "optionally provide a name of the reference ligand",
        ),
        ...
        Option(
            ...
            "bool",
            "False",
            "if -pdb==True method 2: maximum common substructure",
        ),
    ...
    help_text = (
        "Build a Minimum Spanning Tree suggesting the alchemical pairs of ligands to morphe.",
    ...
    if cmdl.opt["-graph"].is_set:
        ...
        # node size
        nsize = [
            (((i - min(nodeDeg)) * GO.opt["nodeSizeSlope"] + min(nodeDeg)))
            * GO.opt["nodeSizeScale"]
            for i in nodeDeg
        ]
        ...
        if cmdl.opt["-ligImg"].is_set == False:
            nx.draw_networkx_nodes(
                ...
                pos=layout,
                node_size=nsize,
                node_color=GO.opt["nodeColor"],
                alpha=GO.opt["nodeTransparency"],
                vmin=GO.opt["nodeVmin"],
                vmax=GO.opt["nodeVmax"],
                cmap=GO.opt["nodeCMAP"],
                linewidths=GO.opt["nodeLineWidth"],
                node_shape=GO.opt["nodeShape"],
            )

        # node labels
        if (GO.opt["nodeLabels"] != None) and (GO.opt["nodeLabels"] != False):
            nodeLabels, nodeLabelLayout = genNodeLabels(
                rmsdMat, nodeNames, layout, GO.opt
            )
            nx.draw_networkx_labels(
                G,
                pos=nodeLabelLayout,
                labels=nodeLabels,
                font_color=GO.opt["nodeFontColor"],
                font_weight=GO.opt["nodeFontWeight"],
                font_size=GO.opt["nodeFontSize"],
                font_family=GO.opt["nodeFontFamily"],
            )

        # draw edges
        if (
            (GO.opt["edgeColorExtra"] != None)
            and (GO.opt["edgeColorExtra"] != False)
            and (GO.opt["edgeColorExtra"] != GO.opt["edgeColor"])
        ):
            G1 = genGraph(rmsdMat, edgeListMain, nx)
            nx.draw_networkx_edges(
                G1,
                pos=layout,
                width=GO.opt["edgeWidth"],
                edge_color=GO.opt["edgeColor"],
                style=GO.opt["edgeStyle"],
                alpha=GO.opt["edgeTransparency"],
                edge_cmap=GO.opt["edgeCMAP"],
                edge_vmin=GO.opt["edgeVmin"],
                edge_vmax=GO.opt["edgeVmax"],
            )
            # extra edges
            if cycleNum > 0:
                ...
                nx.draw_networkx_edges(
                    ...
                    pos=layout,
                    width=GO.opt["edgeWidth"],
                    edge_color=GO.opt["edgeColorExtra"],
                    style=GO.opt["edgeStyle"],
                    alpha=GO.opt["edgeTransparency"],
                    edge_cmap=GO.opt["edgeCMAP"],
                    edge_vmin=GO.opt["edgeVmin"],
        ...
        if (GO.opt["edgeLabels"] != None) and (GO.opt["edgeLabels"] != False):
            ...
            nx.draw_networkx_edge_labels(
                ...
                font_color=GO.opt["edgeFontColor"],
                font_size=GO.opt["edgeFontSize"],
                font_family=GO.opt["edgeFontFamily"],
                font_weight=GO.opt["edgeFontWeight"],
                label_pos=GO.opt["edgeLabelPos"],
                alpha=GO.opt["edgeLabelTransp"],
            )
