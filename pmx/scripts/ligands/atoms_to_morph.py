...
def alignOnSubset(mol1, mol2, constrMap):
    ...
    pyO3A = AllChem.GetCrippenO3A(
        mol1, mol2, mol1_crippen, mol2_crippen, constraintMap=constrMap, options=0
    )  # mol1=probe, mol2=ref
    pyO3A.Align()
    return pyO3A.Score()
...
def checkNeighDist(mol1, mol2, nb1, n1, n2, rem1, rem2):
    ...
    for neigh1 in nb1:
        ...
        if ind1 in n1:
            ...
            if ind2 != None:
                dist = distance_based(
                    mol1, mol2, 0.0, id1=[ind1], id2=[ind2], calcOnly=True
                )
                # 		print ind1,ind2,dist
                if (
                    dist > 0.15
                ):  # 0.15 nm distance should be forgiving enough, but also catch the cases of chirality inversions
...
def localEnvironment(mol, i, n):
    ...
    for a2 in nb1:
        ...
        for a3 in nb2:
            ...
            if (
                (ind2 in n)
                and (ind2 not in list12)
                and (ind2 not in list13)
                and (ind2 != i)
            ):
                list13.append(ind2)
    # final list
    ...
    for i1, i2 in zip(n1, n2):
        ...
        if (str(chirality1) != "CHI_UNSPECIFIED") or (bNonsingleBond1 == True):
            ...
            if len(localEnv1) > 2:
                ...
                Chem.rdMolAlign.AlignMol(
                    mol2, mol1, atomMap=list(zip(localEnv2, localEnv1))
                )
        ...
        if (str(chirality2) != "CHI_UNSPECIFIED") or (bNonsingleBond2 == True):
            ...
            if len(localEnv2) > 2:
                ...
                Chem.rdMolAlign.AlignMol(
                    mol1, mol2, atomMap=list(zip(localEnv1, localEnv2))
                )
            #                alignOnSubset(mol1,mol2,zip(localEnv1,localEnv2))
            # get the neighbours
            nb2 = a2.GetNeighbors()
...
def check_nonsingle_bonds(mol1, mol2, bonds1, n1, n2, dictn1n2):
    for bond in bonds1:
        ...
        # atoms of the bond considered are both in n1, therefore they are also in n2
        if (foo in n1) and (bar in n1):
        ...
        for iEnd in dict_mol1[iStart]:
            ...
            if jStart in list(dict_mol2.keys()):
                if jEnd not in dict_mol2[jStart]:
                    ...
                    if ((jStart in rem_start) or (jStart in rem_end)) and (
                        (jEnd in rem_start) or (jEnd in rem_end)
                    ):
                        continue
                ...
                print(
                    "Warning: something wrong in the 1-2, 1-3 or 1-4 lists. Trying to proceed with the warning..."
                )
                rem_start.append(jStart)
                rem_end.append(jEnd)
...
def checkTop(mol1, mol2, n1, n2, bH2H, bH2heavy):
    ...
    # 3a) fix 1-2
    rem12_mol2_start, rem12_mol2_end = findProblemsExclusions(
        n1, n2, dict12_mol1, dict12_mol2
    )  # output: indeces of mol2
    n2, n1 = fixProblemsExclusions(mol2, mol1, n2, n1, rem12_mol2_start, rem12_mol2_end)
    rem12_mol1_start, rem12_mol1_end = findProblemsExclusions(
        n2, n1, dict12_mol2, dict12_mol1
    )  # output: indeces of mol1
    ...
    # 3b) fix 1-3
    rem13_mol2_start, rem13_mol2_end = findProblemsExclusions(
        n1, n2, dict13_mol1, dict13_mol2
    )  # output: indeces of mol2
    n2, n1 = fixProblemsExclusions(mol2, mol1, n2, n1, rem13_mol2_start, rem13_mol2_end)
    rem13_mol1_start, rem13_mol1_end = findProblemsExclusions(
        n2, n1, dict13_mol2, dict13_mol1
    )  # output: indeces of mol1
    ...
    # 3b) fix 1-4
    rem14_mol2_start, rem14_mol2_end = findProblemsExclusions(
        n1, n2, dict14_mol1, dict14_mol2
    )  # output: indeces of mol2
    n2, n1 = fixProblemsExclusions(mol2, mol1, n2, n1, rem14_mol2_start, rem14_mol2_end)
    rem14_mol1_start, rem14_mol1_end = findProblemsExclusions(
        n2, n1, dict14_mol2, dict14_mol1
    )  # output: indeces of mol1
...
def calcScore(mol1, mol2, n1, n2, bH2H, bH2heavy):
    ...
    res = 1.0 - old_div(res, (na1 + na2 - res))
    return res
...
def o3a_alignment(
    ...
    if bCrippen == True:  # always Crippen
        ...
        if bRingsOnly == True:
            ...
            pyO3A = AllChem.GetCrippenO3A(
                submol1, submol2, submol1_crippen, submol2_crippen, options=0
            )
            rmsd = pyO3A.Align()
            # now align the full molecule
            pyO3A = AllChem.GetCrippenO3A(
                mol1, submol1, mol1_crippen, submol1_crippen, options=0
            )
            pyO3A.Align()
        else:
            pyO3A = AllChem.GetCrippenO3A(
                mol1, mol2, mol1_crippen, mol2_crippen, options=0
            )  # mol1=probe, mol2=ref
            pyO3A.Align()
        # distances
            ...
            for neighb in a.GetNeighbors():
                ...
                if neighb.GetAtomicNum() == 6:
                    if len(neighb.GetNeighbors()) == 2:
                        # need to check bond length (in Angstroms)
                        if getBondLength(mol, a.GetIdx(), neighb.GetIdx()) < 1.25:
                            bTriple = True
    # N
...
def disconnectedMCS(mol1, mol2, ind1, ind2, bH2H=True, bH2Hpolar=True, bH2heavy=True):
    ...
    try:
        ...
        if len(ind1) > 1:
            print(
                "WARNING: the mapping may (but not necessarily) contain disconnected fragments. Proceed with caution."
            )
        return (ind1, ind2)
    ...
    n1_list, n2_list = mcsHremove(
        subMol1, subMol2, n1_list, n2_list, bH2H, bH2Hpolar, bH2heavy
    )
...
def disconnectedRecursive(mol1, mol2, ind1, ind2):
    ...
    for key in matchDict1:
        if len(matchDict1[key]) > maxMatchSize:
            ...
            maxMatchKey = key
            minMatchRMSD = Chem.rdMolAlign.AlignMol(
                mol2, mol1, atomMap=list(zip(matchDict2[key], matchDict1[key]))
            )
...
def mcs(
    ...
    print(
        "Found %d MCSs in total (mol1: %d, mol2: %d), each with %d atoms and %d bonds"
        % (
    ...
    n1_list, n2_list = mcsHremove(
        mol1, mol2, n1_list, n2_list, bH2H, bH2Hpolar, bH2heavy
    )
...
def main(argv):
    ...
    files = [
        ...
        FileOption(
            ...
            ["pdb"],
            "out_pdb1.pdb",
            "optional output of superimposed structure 1",
        ),
        FileOption(
            "-opdb2",
            "w/o",
            ["pdb"],
            "out_pdb2.pdb",
            "optional output of superimposed structure 2",
        ),
        FileOption(
            "-opdbm1",
            "w/o",
            ["pdb"],
            "out_pdb_morphe1.pdb",
            "optional output of the morphable atoms str1",
        ),
        FileOption(
            "-opdbm2",
            "w/o",
            ["pdb"],
            "out_pdb_morphe2.pdb",
            "optional output of the morphable atoms str2",
        ),
        FileOption(
            "-score",
            "w/o",
            ["dat"],
            "out_score.dat",
            "optional output: score of the morph",
        ),
        FileOption(
            "-n1",
            "r/o",
            ["ndx"],
            "scaffold1",
            "optionally read index of atoms to consider: mol1",
        ),
        FileOption(
            "-n2",
            "r/o",
            ["ndx"],
            "scaffold2",
            "optionally read index of atoms to consider: mol2",
        ),
    ]
    ...
    options = [
        ...
        Option(
            ...
            "bool",
            "False",
            "should non-polar hydrogen be morphed into any other hydrogen?",
        ),
        Option(
            "-H2Hpolar",
            "bool",
            "False",
            "should polar hydrogens be morphed into polar hydrogens?",
        ),
        Option(
            "-H2heavy",
            "bool",
            "False",
            "should hydrogen be morphed into a heavy atom (also sets -H2H to true)",
        ),
        Option(
            "-RingsOnly",
            "bool",
            "False",
            "should rings only be used in the MCS search or alignment",
        ),
        Option(
            "-dMCS",
            "bool",
            "False",
            "find MCS, superimpose the structures based on the MCS, apply distance in\
        ...
        Option(
            ...
            "float",
            "0.05",
            "distance (nm) between atoms to consider them morphable",
        ),
    ...
    if cmdl["-RingsOnly"] == True:
        if bYesRings == True:
            ...
            print(
                "-RingsOnly flag is unset, because one (or both) molecule has no rings\n"
            )
    ...
    if (bAlignment == True) or (bMCSfailed == True):
        ...
        print(
            "Alignment is based on atom logP contributions: S. A. Wildman and G. M. Crippen JCICS _39_ 868-873 (1999)\n"
        )
        # only use rings
        if bRingsOnly == True:
            n1align, n2align, pyO3A = o3a_alignment(
                mol1,
                mol2,
                bH2H,
                bH2Hpolar,
    ...
    print(
        "To ensure that the mapping is symmetric, i.e. mol1->mol2=mol2->mol1, we repeat the same procedure by swapping the molecule order.\n"
    )
    print(
        "********************************************************************************************************************************"
    )
    ...
    if (bAlignment == True) or (bMCSfailed == True):
        print("\nRunning alignment:")
        if bRingsOnly == True:
            n2align, n1align, pyO3A = o3a_alignment(
                mol2,
                mol1,
                bH2H,
                bH2Hpolar,
    ...
    if (len(n1align) >= len(n1mcs)) and (len(n1align) > len(n1)):
        print(
            "The final result is based on the O3A alignment after swapping the molecule order (mol2-mol1)."
        )
        n1 = n1align
        n2 = n2align
    elif (len(n1align) < len(n1mcs)) and (len(n1mcs) > len(n1)):
        print(
            "The final result is based on the MCS after swapping the molecule order (mol2-mol1)."
        )
        n1 = n1mcs
        n2 = n2mcs
    print("\n")
    ...
    if cmdl.opt["-opdb2"].is_set:
        ...
        try:
            ...
            print(
                "Cannot superimpose -opdb2 structure. Maybe no morphable atoms have been found\n"
            )
