...
def disconnectedMCS(mol1,mol2,ind1,ind2,bH2H=True,bH2Hpolar=True,bH2heavy=True):
    ...
    res = rdFMCS.FindMCS([subMol1,subMol2],ringMatchesRingOnly=True, completeRingsOnly=True, atomCompare='any', bondCompare='any')
...
def mcs(mol1, mol2, molForMcs1,molForMcs2, bH2H, bH2Hpolar, bH2heavy, bdMCS, bRingsOnly, d, bChiral, sigmaHoleID1, sigmaHoleID2, t=None):
    ...
    mols = [foo,bar]
    print("Searching...")
#    res =rdFMCS.FindMCS(mols,ringMatchesRingOnly=True, completeRingsOnly=True, atomCompare='elements', bondCompare='any', timeout=int(t), maximize='bonds')
    # for new RDKit-2018 use below
...
def getImage(mol, mol2, atomset1=None, atomset2=None):    
    AllChem.Compute2DCoords(mol)
    AllChem.Compute2DCoords(mol2)
    if atomset1 is not None:
        #highlight the atoms set while drawing the whole molecule.
        img = MolsToGridImage([mol, mol2], molsPerRow=2, subImgSize=(600, 600), highlightAtomLists=[atomset1, atomset2])
    else:
        img = MolsToGridImage([mol, mol2], molsPerRow=2, subImgSize=(600, 600))
    return img
