...
def gen_dih_entry2(
    a1, a2, a3, a4, dihtype, entry=None, entryB=None, scDumA=1.0, scDumB=1.0
):
    dihList = []
    ...
    if entryB != None:
        ...
        dih = (
            [a1, a2, a3, a4] + [dihtype] + [[dihtype] + zeroesB] + [[dihtype] + entryB]
        )
        dihList.append(dih)
    #    if( entry==None ):
...
def main(argv):
    ...
    options = [
        ...
        Option(
            ...
            "bool",
            "false",
            "fit mol2 onto mol1, only works if pairs.dat is provided",
        ),
        Option(
    ...
    if read_from_idx:
        ...
        for b in itp1.bonds:
            if b[0] in grp1.ids and b[1] not in grp1.ids:
                grp1.ids.append(b[1])
                do_log(
                    logfile, "Adding atom %s to scaffold 1" % m1.atoms[b[1] - 1].name
                )
            elif b[1] in grp1.ids and b[0] not in grp1.ids:
                grp1.ids.append(b[0])
                do_log(
                    logfile, "Adding atom %s to scaffold 1" % m1.atoms[b[0] - 1].name
                )
        for b in itp2.bonds:
            if b[0] in grp2.ids and b[1] not in grp2.ids:
                grp2.ids.append(b[1])
                do_log(
                    logfile, "Adding atom %s to scaffold 2" % m2.atoms[b[1] - 1].name
                )
            elif b[1] in grp2.ids and b[0] not in grp2.ids:
                grp2.ids.append(b[0])
                do_log(
                    logfile, "Adding atom %s to scaffold 2" % m2.atoms[b[0] - 1].name
                )
    ...
    m3 = (
        m1.copy()
    )  # m3 will contain all the atoms from m1, but with the coordinates of the matching atoms from m2
    ...
    for a1, a2 in pairs:
        ...
        a2.atomtypeB = (
            a1.atomtype
        )  # this is my change to catch the DISAPPEARING dihedrals
        a1.nameB = a2.name
        a1.qB = a2.q
        a1.mB = a2.m
        a1.idB = a2.id
        ...
        do_log(
            logfile,
            "Atom....: %4d  %12s | %6.2f | %6.2f -> %12s | %6.2f | %6.2f"
    ...
    if bFit == False:
        for atom in dumsA:
            ...
            do_log(
                logfile,
                "Dummy...: %4d  %12s | %6.2f | %6.2f -> %12s | %6.2f | %6.2f"
                % (
                    atom.id,
                    atom.atomtype,
                    atom.q,
    ...
    for b in itp1.bonds:
        ...
        if not bOk:
            ...
            do_log(
                logfile,
                "A-> Atom1: %d-%s Atom2: %d-%s" % (a1.id, a1.name, a2.id, a2.name),
            )
            do_log(
                logfile,
                "B-> Atom1: %d-%s Atom2: %d-%s" % (a1.idB, a1.nameB, a2.idB, a2.nameB),
            )
            do_log(logfile, "Exiting....")
            sys.exit(1)
    ...
    for b in itp1.angles:
        ...
        if hasattr(a1, "idB") and hasattr(a2, "idB") and hasattr(a3, "idB"):
            ...
            if entr is not None:
                if angtype == 1:
                    newangles.append(
                        [b[0], b[1], b[2], angtype, [angtype] + b[4], [angtype] + entr]
                    )
                else:
            ...
            a1.atomtypeB[:3] == "DUM"
            or a2.atomtypeB[:3] == "DUM"
            or a3.atomtypeB[:3] == "DUM"
        ):
            entr = get_ff2_entry([a1.id, a2.id, a3.id], itp1, gmx45, what="angle")
            if (angtype != 1) and (
                entr is not None
            ):  # remove the angtype from the entr
                entr = entr[1:]
            if entr is not None:
                if (
                    (a1.atomtypeB[:3] != "DUM" and a2.atomtypeB[:3] != "DUM")
                ...
                if angtype == 1:
                    ...
                    newangles.append(
                        [b[0], b[1], b[2], angtype, b[4], [angtype] + entr]
                    )
                bOk = True
            else:
                bOk = False
        ...
        if not bOk:
            ...
            do_log(
                logfile,
                "A-> Atom1: %d-%s Atom2: %d-%s Atom3: %d-%s"
                % (a1.id, a1.name, a2.id, a2.name, a3.id, a3.name),
            )
            do_log(
                logfile,
                "B-> Atom1: %d-%s Atom2: %d-%s Atom3: %d-%s"
    ...
    for b in itp1.dihedrals:
        ...
        a4 = m1.atoms[id4 - 1]
        entrA = get_ff2_entry(
            [id1, id2, id3, id4, dih_type], cpItp1, gmx45, what="dihedral"
        )
        bOk = False
        if (
            hasattr(a1, "idB")
            ...
            if (
                ...
                if entrA is not None:
                    ...
                    if (
                        ...
                        if (
                            dih_type == 2 or dih_type == 4 or dih_type == 1
                        ):  # disable improper for dummy-nondummy
                            dih = gen_dih_entry2(
                                a1, a2, a3, a4, dih_type, None, entrA, 1.0, 0.0
                            )
                        else:
                            dih = gen_dih_entry2(
                                a1, a2, a3, a4, dih_type, None, entrA, 1.0, scDumd
                            )
                    else:
        ...
        if not bOk:
            ...
            do_log(
                logfile,
                "A-> Atom1: %d-%s Atom2: %d-%s Atom3: %d-%s Atom3: %d-%s"
                % (a1.id, a1.name, a2.id, a2.name, a3.id, a3.name, a4.id, a4.name),
            )
            do_log(
                logfile,
                "B-> Atom1: %d-%s Atom2: %d-%s Atom3: %d-%s Atom3: %d-%s"
                % (
                    a1.idB,
                    a1.nameB,
                    a2.idB,
    ...
    for b in itp2.dihedrals:
        ...
        dih_type = b[4]
        entrB = get_ff2_entry(
            [b[0].id, b[1].id, b[2].id, b[3].id, dih_type],
            cpItp2,
            gmx45,
            what="dihedral",
        )
        ...
        if (
            ...
            if (
                a1.atomtype.startswith("DUM")
                or a2.atomtype.startswith("DUM")
                or a3.atomtype.startswith("DUM")
                or a4.atomtype.startswith("DUM")
            ):
                if entrB is not None:
                    ...
                    if (
                        a1.atomtype.startswith("DUM") == False
                        or a2.atomtype.startswith("DUM") == False
                        or a3.atomtype.startswith("DUM") == False
                        or a4.atomtype.startswith("DUM") == False
                    ):
                        ...
                        if (
                            ...
                            dih = gen_dih_entry2(
                                a1, a2, a3, a4, dih_type, entrB, None, 0.0, 1.0
                            )
                        else:
                            dih = gen_dih_entry2(
                                a1, a2, a3, a4, dih_type, entrB, None, scDumd, 1.0
                            )
                    else:
    ...
    for b in itp2.bonds:
        ...
        a1 = m1.atoms[newid1 - 1]
        a2 = m1.atoms[newid2 - 1]
        if a1.atomtype.startswith("DUM") or a2.atomtype.startswith("DUM"):
    ...
    for b in itp2.angles:
        ...
        if angtype == 5:
            ...
            paramAscDum = [
                5,
                entry[1],
                scDuma * float(entry[2]),
                entry[3],
                scDuma * float(entry[4]),
            ]
        ...
        if (
            ...
            if (
                (
                    a1.atomtype.startswith("DUM") == False
                    and a2.atomtype.startswith("DUM") == False
                )
                or (
                    a1.atomtype.startswith("DUM") == False
                    and a3.atomtype.startswith("DUM") == False
                )
                or (
                    a2.atomtype.startswith("DUM") == False
                    and a3.atomtype.startswith("DUM") == False
                )
            ):
    ...
    for p in itp2.pairs:
        ...
        a1 = m1.atoms[newid1 - 1]
        a2 = m1.atoms[newid2 - 1]
        if (newid1, newid2) not in pp and (newid2, newid1) not in pp:
