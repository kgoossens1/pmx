...
class RTPParser(object):
    ...
    def write(self, out_file):
        ...
        for key in self.keys:
            ...
            for atom in entr["atoms"]:
                print(
                    "%6s   %-15s  %8.5f  %d" % (atom[0], atom[1], atom[2], atom[3]),
                    file=fp,
                )
            if entr["bonds"]:
                print(" [ bonds ]", file=fp)
            ...
            if entr["improps"]:
                ...
                for dih in entr["improps"]:
                    try:
                        ...
                        print(
                            "%6s  %6s  %6s  %6s " % (dih[0], dih[1], dih[2], dih[3]),
                            file=fp,
                        )
            print(file=fp)
    ...
    def __check_residue_tree(self, model):
        for c in model.chains:
            if not c.residue_tree_ok:
                print(
                    "pmx_Error_> Broken residue tree in chain ", c.id, file=sys.stderr
                )
                sys.exit(1)
            ...
            if residue.resname in ["CYS2", "CCYX"]:
                for r in model.residues:
                    if r.resname in ["CYS2", "CCYX"] and r.id < residue.id:
                        ...
                        if d < 2.5:
                            print(
                                "pmx__> Disulfid bond between residue",
                                sg1.resnr,
                                "and",
                                sg2.resnr,
                                file=sys.stderr,
    ...
    def __find_rtp_dihedral(self, key, name1, name2, name3, name4):
        ...
        for d in rtp_d:
            if (
                d[0] == name1 and d[1] == name2 and d[2] == name3 and d[3] == name4
            ) or (d[0] == name4 and d[1] == name3 and d[2] == name2 and d[3] == name1):
                return d
        return None
...
class BondedParser(object):
    ...
    def assign_params(self, model):
        ...
        for angle in model.angle_list:
            params = self.get_angle_param(
                angle[0].bond_type, angle[1].bond_type, angle[2].bond_type
            )
            angle.extend(params)
        for dih in model.dihedral_list:
