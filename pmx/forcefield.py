...
class ITPFile(object):
    ...
    def write_itp_vsites2(self, fp):
        ...
        for v in self.virtual_sites2:
            print(
                "%8d %8d %8d %s %s" % (v[0].id, v[1].id, v[2].id, v[3], v[4]), file=fp
            )
    ...
    def write_rtp(self, filename="mol.rtp"):
        ...
        for atom in self.atoms:
            print(
                "%8s %-12s %8.6f %5d" % (atom.name, atom.atomtype, atom.q, atom.cgnr),
                file=fp,
            )
        ...
        for bond in self.bonds:
            if len(bond) <= 3:
                print("%8s %8s " % (bond[0].name, bond[1].name), file=fp)
            else:
                print(
                    "%8s %8s %8.4f %8.4f "
        ...
        for angle in self.angles:
            if len(angle) <= 4:
                print(
                    "%8s %8s %8s " % (angle[0].name, angle[1].name, angle[2].name),
                    file=fp,
                )
            elif angle[3] == 5:  # U-B
                print(
                    "%8s %8s %8s %8.4f %8.4f %8.4f %8.4f "
                    % (
                        angle[0].name,
                        angle[1].name,
                        angle[2].name,
        ...
        for dih in self.dihedrals:
            if len(dih) <= 5:  # no parameters
                ...
                print(
                    "%8s %8s %8s %s %8.4f %8.4f %8.4f %8.4f %8.4f %8.4f "
                    % (
                        dih[0].name,
                        dih[1].name,
                        dih[2].name,
...
class Topology(object):
    ...
    def make_Bstates(self, subset=None):
        ...
        for i, b in enumerate(self.bonds):
            if len(b) > 3:
                if self.bonds[i][0] in atomlist or self.bonds[i][1] in atomlist:
                    param = copy.deepcopy(b[-1])
                    self.bonds[i].append(param)
        for i, ang in enumerate(self.angles):
            if len(ang) > 4:
                if (
                    self.angles[i][0] in atomlist
                    or self.angles[i][1] in atomlist
                    or self.angles[i][2] in atomlist
                ):
        ...
        for i, dih in enumerate(self.dihedrals):
            if len(dih) > 5:
                if (
                    self.dihedrals[i][0] in atomlist
                    or self.dihedrals[i][1] in atomlist
                    or self.dihedrals[i][2] in atomlist
                    or self.dihedrals[i][3] in atomlist
                ):
    ...
    def read_bonds(self, lines):
        ...
        for line in lst:
            ...
            if len(entries) == 3:
                ...
                k = float(entries[4])
                self.bonds.append(
                    [self.atoms[idx[0] - 1], self.atoms[idx[1] - 1], idx[2], [l, k]]
                )
    ...
    def __last_perturbed_atom(self, r):
        ...
        last_atom = None
        for atom in r.atoms:
            if self.__atoms_morphe([atom]) and atom.name not in [
                "N",
                "CA",
                "C",
                "O",
                ...
                if not atom.atomtype.startswith(
                    "DUM"
                ) and not atom.atomtypeB.startswith("DUM"):
                    last_atom = atom
        if last_atom == None:
            print(
                "Error: Could not find a perturbed atom to put rest charges on !",
                file=sys.stderr,
            )
            sys.exit(1)
        return last_atom
    ...
    def write_atoms(
        ...
        for r in self.residues:
            if self.__is_perturbed_residue(r):
                ...
                print(
                    "Making target charge %g for residue %s"
                    ...
                    if self.__atoms_morphe([atom]):
                        if (
                            ...
                            if not full_morphe and (
                                atom.q * atom.qB < 0 or atom.atomtype != atom.atomtypeB
                            ):
                                atom.qqB = 0
                            ...
                            if not full_morphe:
                                if hasattr(atom, "contQ"):
                                    atom.qqA = atom.contQ
                                    atom.qqB = atom.qqA
                                # this a big q morphe. has been set to zero before
                                if hasattr(atom, "to_be_morphed"):
                                    if atomtypes == "BB":
                                        ...
                                        atom.qqA = 0
                                        atom.qqB = 0
                                elif not hasattr(atom, "contQ") and not hasattr(
                                    atom, "to_be_morphed"
                                ):
                                    atom.qqA = atom.qB
                                    atom.qqB = atom.qB
                                ...
                                atom.qqA = atom.qB
                                atom.qqB = atom.qB
                        if atom.atomtype.startswith("DUM") or atom.atomtypeB.startswith(
                            "DUM"
                        ):
                            if dummy_qA == "off":
                                atom.qqA = 0.0
                ...
                if qB_tot != target_chargeB:
                    ...
                    print(
                        "Selecting atom %d-%s (%s) as perturbed atom with highest order"
                        % (latom.id, latom.name, latom.resname)
                    )
                    newqB = latom.qqB - (qB_tot - target_chargeB)
                    print(
                        "Changing chargeB of atom %s from %g to %g"
        ...
        for atom in al:
            if self.__atoms_morphe([atom]):
                ...
                if hasattr(atom, "qqB"):
                    ...
                    qqB = atom.qB
                print(
                    "%6d%11s%7d%7s%7s%7d%11.6f%11.4f%11s%11.6f%11.4f"
                    % (
                        atom.id,
                        atA,
                        atom.resnr,
                ...
                print(
                    "%6d%11s%7d%7s%7s%7d%11.6f%11.4f"
                    % (
                        atom.id,
                        atom.atomtype,
                        atom.resnr,
    ...
    def write_bonds(self, fp, state="AB"):
        ...
        for b in self.bonds:
            if len(b) == 3:
                ...
                if state == "AB":
                    print(
                        "%6d %6d %6d %14.6f %14.6f %14.6f %14.6f"
    ...
    def write_angles(self, fp, state="AB"):
        ...
        for ang in self.angles:
            if len(ang) == 4:
                print(
                    "%6d %6d %6d %6d" % (ang[0].id, ang[1].id, ang[2].id, ang[3]),
                    file=fp,
                )
            else:
                if state == "AB":
                    print(
                        "%6d %6d %6d %6d %14.6f %14.6f %14.6f %14.6f ; %s %s %s"
                        % (
                            ang[0].id,
                            ang[1].id,
                            ang[2].id,
    ...
    def write_dihedrals(self, fp, state="AB"):
        ...
        for d in self.dihedrals:
            if len(d) == 5:
                print(
                    "%6d %6d %6d %6d %4d" % (d[0].id, d[1].id, d[2].id, d[3].id, d[4]),
                    file=fp,
                )
            elif len(d) == 6:
                print(
                ...
                if state == "AB":
                    print(
                        "%6d %6d %6d %6d %4d %s %s ; %s %s %s %s %s %s %s %s (%s->%s)"
                        % (
                            d[0].id,
                            d[1].id,
                            d[2].id,
    ...
    def make_bond_params(self):
        for i, (at1, at2, func) in enumerate(self.bonds):
            ...
            if param is None:
                print(
                    "Error! No bonded parameters found! (%s-%s)" % (at1.type, at2.type)
                )
                sys.exit(1)
    ...
    def make_angle_params(self):
        for i, (at1, at2, at3, func) in enumerate(self.angles):
            ...
            if param is None:
                print(
                    "Error! No angle parameters found! (%s-%s-%s)"
    ...
    def make_dihedral_params(self):
        for i, d in enumerate(self.dihedrals):
            if d[5] != "":  # we have a prefefined dihedral
                ...
                param = get_dihedral_param(
                    at1.type, at2.type, at3.type, at4.type, self.dih_lib, func
                )
                if param is None:
                    print(
                        "Error! No dihedral parameters found! (%s-%s-%s-%s)"
...
class MDP(object):
    ...
    def __setitem__(self, item, value):
        if item not in self.parameters:
            raise_(MDPError, "No such option %s" % item)
        ...
        if chain.residues[0].is_protein_residue():
            ...
            try:
                ...
                try:
                    ...
                    print(
                        "pymacs_Warning_> No terminal oxygen atoms found in chain %s"
                        % chain.id,
                        file=sys.stderr,
                    )
