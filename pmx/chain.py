...
class Chain(Atomselection):
    ...
    def insert_residue(self, pos, mol, newResNum=False):
        ...
        if pos not in list(range(len(self.residues) + 1)):
            raise_(ValueError, "Chain has only %d residues" % len(self.residues))
        else:
            mol.set_resid(-999)
            if newResNum != False:
    ...
    def append(self, mol):
        if not isinstance(mol, Molecule):
            raise_(TypeError, "%s is not a Molecule instance" % str(mol))
        else:
            n = len(self.residues)
    ...
    def attach_chain(self, newchain, phi=-139.0, psi=135.0):
        if self.unity == "nm":
            self.nm2a()
            changed = True
        else:
    ...
    def cbuild(self, resn, next_phi=-139.0, psi=135.0):
        if len(resn) == 1:
        ...
        if new.resname == "PRO":
            ...
            cr2 = cross(v3, v4)
            x = 1.0 / linalg.norm(cr2)
            cr2 = cr2 * x
            d = dot(cr, cr2)
            dd = arccos(dot(cr, cr2))
            v5 = cross(cr, cr2)
            x = 1.0 / linalg.norm(v5)
            v5 = v5 * x
            rv = N.x + v5
            r = Rotation(rv, N.x)
            for atom in new.atoms:
    ...
    def nbuild(self, resn, phi=-139.0, psi=135.0):
