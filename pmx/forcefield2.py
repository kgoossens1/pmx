...
def cpp_parse_file(
    fn, cpp_defs=[], cpp_path=[os.environ.get("GMXLIB")], itp=False, ffpath=None
):
...
class TopolBase(object):
            ...
            if len(ang) == 4:
                ...
                if state == "A":
                    if ang[3] == 1:
                        ...
                        if shape(ang[4])[0] == 4:
                            print(
                                "%6d %6d %6d %6d %14.6f %14.6f %14.6f %14.6f"
                                % (
                                    ang[0].id,
                                    ang[1].id,
                                    ang[2].id,
                ...
                if state == "AB":
                    ...
                    if ang[3] == 1:
                        ...
                        if len(ang) > 5:  # B-state exists
                            print(
                                "%6d %6d %6d %6d %14.6f %14.6f %14.6f %14.6f %14.6f %14.6f %14.6f %14.6f ; %s %s %s"
                                % (
                                    ang[0].id,
                                    ang[1].id,
                                    ang[2].id,
    ...
    def write_vsites2(self, fp):
        ...
        for vs in self.virtual_sites2:
            if len(vs) == 4:
                print(
                    "%6d %6d %6d %4d" % (vs[0].id, vs[1].id, vs[2].id, vs[3]), file=fp
                )
            elif len(vs) == 5:
                print(
                    "%6d %6d %6d %4d %s" % (vs[0].id, vs[1].id, vs[2].id, vs[3], vs[4]),
                    file=fp,
                )
            else:
                sys.stderr.write(
    ...
    def write_vsites3(self, fp):
        ...
        for vs in self.virtual_sites3:
            if len(vs) == 5:
                ...
                print(
                    "%6d %6d %6d %6d %4d %s"
                    % (vs[0].id, vs[1].id, vs[2].id, vs[3].id, vs[4], vs[5]),
                    file=fp,
                )
            else:
                sys.stderr.write(
    ...
    def write_vsites4(self, fp):
        ...
        for vs in self.virtual_sites4:
            if len(vs) == 6:
                ...
                print(
                    "%6d %6d %6d %6d %6d %4d %s"
                    % (vs[0].id, vs[1].id, vs[2].id, vs[3].id, vs[4].id, vs[5], vs[6]),
                    file=fp,
                )
            else:
                sys.stderr.write(
    ...
    def write_posre(self, fp):
        ...
        for pr in self.posre:
            if len(pr) == 3:
                ...
                sys.stderr.write(
                    "EEK! Something went wrong while writing position_restraints!!!!\n"
                )
                print(pr)
                sys.exit(1)
    ...
    def write_ii(self, fp):
        ...
        if "dihedrals" in list(self.ii.keys()):
            ...
            for dih in self.ii["dihedrals"]:
                fp.write(
                    "%6d %6d %6d %6d %6d"
                    % (dih[0].id, dih[1].id, dih[2].id, dih[3].id, dih[4])
                )
                if len(dih) > 5:
                    for x in dih[5]:
                        fp.write(" %14.6f" % x)
    ...
    def __atoms_morphe(self, atoms):
        for atom in atoms:
            if atom.atomtypeB is not None and (
                atom.q != atom.qB
                or atom.m != atom.mB
                or atom.atomtype != atom.atomtypeB
            ):
                return True
        return False
...
class Topology(TopolBase):
    ...
    def __init__(
        ...
        if assign_types:
            l = cpp_parse_file(
                topfile, cpp_defs=cpp_defs, cpp_path=cpp_path, itp=bItp, ffpath=ffpath
            )
            l = kickOutComments(l, "#")
            l = kickOutComments(l, ";")
        ...
        if chain.residues[0].is_protein_residue():
            ...
            try:
                ...
                try:
                    ...
                    print(
                        "pmx_Warning_> No terminal oxygen atoms found in chain %s"
                        % chain.id,
                        file=sys.stderr,
                    )
